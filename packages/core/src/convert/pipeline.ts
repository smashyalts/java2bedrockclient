...
function buildDisplayEntityYaml(
  entries: { key: string; type: string; identifier: string; modelData?: number; yOffset?: number }[],
): string {
  const lines = [
    ...
    "#",
    "# y-offset is derived per item from the model's vertical centre so tall",
    "# furniture doesn't float (the extension's flat -0.5 only seats ~1-block",
    "# items). If a piece still floats, lower its y-offset; if it sinks, raise it.",
